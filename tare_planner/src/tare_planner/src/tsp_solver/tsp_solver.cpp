...
TSPSolver::TSPSolver(tsp_solver_ns::DataModel data)
  : data_(std::move(data)), manager_(data_.distance_matrix.size(), data_.num_vehicles, data_.depot), routing_(manager_)
{
}
...
{
  // Inspect solution.
  std::cout << "Objective: " << (solution_->ObjectiveValue()) / 10.0 << " meters" << std::endl;
