...
static bool finiteTriple(double x, double y, double z) {
    return finiteValue(x) && finiteValue(y) && finiteValue(z);
}
...
void IOROS::recvState(LowlevelState *state){
    for(int i(0); i < 12; ++i){
        if (finiteValue(_lowState.motorState[i].q) &&
            finiteValue(_lowState.motorState[i].dq) &&
            finiteValue(_lowState.motorState[i].ddq) &&
            finiteValue(_lowState.motorState[i].tauEst)) {
            ...
            ROS_WARN_THROTTLE(1.0, "Ignoring invalid motor state sample from Gazebo");
        }
    }
    for(int i(0); i < 3; ++i){
...
{ 
    if (!finiteValue(msg.orientation.w) ||
        !finiteTriple(msg.orientation.x, msg.orientation.y, msg.orientation.z) ||
        !finiteTriple(msg.angular_velocity.x, msg.angular_velocity.y, msg.angular_velocity.z) ||
        !finiteTriple(msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z)) {
        ROS_WARN_THROTTLE(1.0, "Ignoring invalid IMU sample from Gazebo");
        return;
    }
