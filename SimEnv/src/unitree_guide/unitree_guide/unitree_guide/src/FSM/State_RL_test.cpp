                ...
                :FSMState(ctrlComp, FSMStateName::RL, "RL")
{
    ...
    rlDebugPub = nh.advertise<std_msgs::Float32MultiArray>("/unitree_rl_debug", 1);
...
void State_RL::exit(){
    ...
    if (debug == true && amp_obs_thread != nullptr) {
...
void State_RL::infer_thread_callback()
{
    while(infer_thread_runnning == State_RL::RUNNING)
    {
        ...
        float target_delta_abs_sum = 0.0f;
        float target_q0 = 0.0f;
        float target_q1 = 0.0f;
        float target_q2 = 0.0f;
        if (debug == true) std::cout << "actions[reindex[j]]  + default_dof_pos" << std::endl;
        for(int i=0; i<12; i++){
            action_abs_sum += std::abs(actions[i]);
            const float target_q = actions[reindex[i]]  + default_dof_pos_tensor[reindex[i]].item<float>();
            target_delta_abs_sum += std::abs(target_q - static_cast<float>(_lowState->motorState[i].q));
            if (i == 0) target_q0 = target_q;
            if (i == 1) target_q1 = target_q;
            if (i == 2) target_q2 = target_q;
            if (real == false)
            {
                _lowCmd->motorCmd[i].q = target_q;
                _lowCmd->motorCmd[i].Kp = 80;
                _lowCmd->motorCmd[i].Kd = 1;
            }
            else if (real == true)
        ...
        if (now - lastDebugPubTime > 500000) {
            std_msgs::Float32MultiArray debugMsg;
            debugMsg.data = {
                static_cast<float>(current_cmd_vel_.linear_x),
                static_cast<float>(current_cmd_vel_.linear_y),
                static_cast<float>(current_cmd_vel_.angular_z),
                action_abs_sum,
                actions.empty() ? 0.0f : actions[0],
                actions.empty() ? 0.0f : actions[1],
                actions.empty() ? 0.0f : actions[2],
                static_cast<float>(commands_tensor[0].item<float>()),
                static_cast<float>(commands_tensor[1].item<float>()),
                static_cast<float>(commands_tensor[2].item<float>()),
                target_delta_abs_sum,
                target_q0,
                target_q1,
                target_q2
            };
            rlDebugPub.publish(debugMsg);
            lastDebugPubTime = now;
        }
